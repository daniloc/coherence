...
//
// The pure core (assignLots, deriveClaimed/Gates/Light, mergeSceneDiff) is exported so the
// derivation is testable without a repo; buildSceneModel and deriveBaseModel are the thin
// IO shells (layout read/write, git stamp, git churn, base worktree, disk reads) around it.
// The genuinely SHARED utilities (fileStats, claimedFilePaths, withBaseWorktree, the
// outside tally) live in src/tree.ts — mass/economy/promise read the same seams.
