...
_files:_ `atlas.ts`, `boundary.ts`, `calibration.ts`, `cli.ts`, `commands.ts`, `config.ts`, `context.ts`, `contracts.ts`, `control.ts`, `conventions.ts`, `decisions.ts`, `decompose.ts`, `derive.ts`, `doctrine.ts`, `drift.ts`, `due.ts`, `economy.ts`, `evolution.ts`, `floor.ts`, `hook-cli.ts`, `hooks.ts`, `index-model.ts`, `journal.ts`, `lint-sinks.ts`, `mass.ts`, `novelty.ts`, `observed.ts`, `oracle-domain.ts`, `panel.ts`, `parity.ts`, `phrasebook.ts`, `premise.ts`, `promise-model.ts`, `promise.ts`, `prose.ts`, `raise.ts`, `read-trace.ts`, `redundancy.ts`, `regulate.ts`, `render-claude.ts`, `render-contract.ts`, `render-index.ts`, `render-outline.ts`, `render-overview.ts`, `run-named-test.ts`, `scaffold.ts`, `sidecar.ts`, `signal.ts`, `status.ts`, `structural.ts`, `test-batch.ts`, `tree.ts`, `types.ts`, `verify.ts`, `walk.ts`, `why-lint.ts`
